...
import {createPost, updatePost} from '../../actions/askFeed';
...
(currentId?state.askFeed.posts.find((p)=>p._id===currentId):null));
...
const clear = ()=>{
 ...
 if(currentId===0){
  ...
  const body = (
    <Card className={classes.paper} style={{background:"white",width:"50%",position:"relative",top:"11%",left:"50%",transform:"translateX(-50%)"}}>
     <Typography variant="h6" align="center" style={{fontWeight: 'bold'}}>Create Post</Typography>
        <CardHeader
        ...
        avatar={
          ...
          <Grid container spacing={1}>
                ...
                renderInput={(params) => (
                  <TextField
                  {...params}
                    label="Select Community" variant="outlined"  value={params} onChange={(e)=>setPostData({...postData, community:params})}
                  />
                )}
              /> */}
               <Select
        ...
        >
          ...
          {options.map((opt)=>(
            <option value={opt}>{opt}</option>
               ...
               <h4 >
                  Post will be visible only to the selected Community
                </h4>
                </Grid>
               <Grid item xs={12}> 
              <div >
