...
function TimeTable(props) {
  ...
  const table=useSelector((state)=>{return state.table})
  const day = [
    "monday",
    "tuesday",
    "wednesday",
    ...
    <div className="timeTableContainer" style={{display: props.props[0]?"block":"none", background: "white",overflowX:"scroll" }}>
      ...
      {day.map((item) => {
        ...
        <div className="container1" style={{display:"flex",flexDirection:"column"}}>
            ...
            <Grid item xs={6}>
            <div style={{ position: "relative", margin: "1.5em" }} >
              <div></div>
                <label
                    style={{
                        position: "absolute",
          ...
          <div className="container">
            <Days table={table} item={item}/>
            
          </div>
          </>
        );
      })}
      <button onClick={update_time}>submit</button>
    </div>
  );
}
